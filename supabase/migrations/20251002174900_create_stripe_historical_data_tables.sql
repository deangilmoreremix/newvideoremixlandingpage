-- Migration: Create Stripe Historical Data Tables
-- This stores complete historical data from Stripe for analytics and customer profiles
...
CREATE TABLE stripe_charges (
  ...
  created BIGINT,
  currency TEXT,
  customer TEXT REFERENCES stripe_customers(id),
...
CREATE TABLE stripe_prices (
  ...
  metadata JSONB,
  nickname TEXT,
  product TEXT REFERENCES stripe_products(id),
  recurring JSONB,
  tax_behavior TEXT,
  tiers JSONB,
  tiers_mode TEXT,
...
LEFT JOIN (SELECT customer_id, COUNT(*) as count FROM stripe_payment_methods GROUP BY customer_id) pm_count
  ON c.id = pm_count.customer_id
LEFT JOIN (SELECT customer, SUM(amount) as lifetime_value FROM stripe_charges WHERE status = 'succeeded' GROUP BY customer) lv
  ON c.id = lv.customer
LEFT JOIN (SELECT customer, COUNT(*) as payment_count FROM stripe_charges WHERE status = 'succeeded' GROUP BY customer) pc
  ON c.id = pc.customer
LEFT JOIN (SELECT customer, MIN(created) as first_purchase_date FROM stripe_charges WHERE status = 'succeeded' GROUP BY customer) fp
  ON c.id = fp.customer
LEFT JOIN (SELECT customer, MAX(created) as latest_purchase_date FROM stripe_charges WHERE status = 'succeeded' GROUP BY customer) lp
  ON c.id = lp.customer
LEFT JOIN (SELECT customer, COUNT(*) as active_subscriptions FROM stripe_subscriptions WHERE status = 'active' GROUP BY customer) subs
  ON c.id = subs.customer
LEFT JOIN (SELECT customer, COUNT(*) as invoice_count FROM stripe_invoices GROUP BY customer) inv
...
FROM stripe_customers c
JOIN stripe_charges ch ON c.id = ch.customer
LEFT JOIN stripe_invoices i ON ch.invoice = i.id
LEFT JOIN stripe_prices pr ON i.lines->0->>'price' = pr.id
LEFT JOIN stripe_products p ON pr.product = p.id
...
CREATE INDEX idx_stripe_customers_created ON stripe_customers(created);
CREATE INDEX idx_stripe_charges_customer ON stripe_charges(customer);
CREATE INDEX idx_stripe_charges_created ON stripe_charges(created);
CREATE INDEX idx_stripe_invoices_customer ON stripe_invoices(customer);
CREATE INDEX idx_stripe_subscriptions_customer ON stripe_subscriptions(customer);
CREATE INDEX idx_stripe_checkout_sessions_customer ON stripe_checkout_sessions(customer);
...
CREATE POLICY "Stripe customers admin only" ON stripe_customers FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe payment methods admin only" ON stripe_payment_methods FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe charges admin only" ON stripe_charges FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe invoices admin only" ON stripe_invoices FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe subscriptions admin only" ON stripe_subscriptions FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe products admin only" ON stripe_products FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe prices admin only" ON stripe_prices FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe checkout sessions admin only" ON stripe_checkout_sessions FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);

CREATE POLICY "Stripe payment intents admin only" ON stripe_payment_intents FOR ALL USING (
  EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin')
);
